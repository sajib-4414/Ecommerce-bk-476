...
def order_completed_event(sender, order_id, cart_id, **kwargs):
    print("Observer Noted that order completed, order ID is::"
          +str(order_id)+", cart ID is::"+str(cart_id))

    '''
    We got a signal order complete, so we will decrease the products quantity 
    and delete the cart items here
    '''
